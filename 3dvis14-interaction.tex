...
%% Abstract section.
\abstract{ 
In this position paper we discuss the usage of various interaction technologies with focus on interactive presentation of scientific data sets for scenarios involving a presenter and an audience. While an interaction technique is commonly evaluated from a user perceptive, we want to shift the focus from a sole analysis of the naturalness and the ease-of-use for the user, to focus on how expressive and understandable the interaction technique is when witnessed by the audience.
...
Thus, while some natural interaction techniques for interactive visualization are easy to perform by the presenter, they may be less beneficial when interacting with the visualization in front of an audience
...
We argue that interaction techniques complement the visualization in a interactive presentation scenario as they also serve as an important communication channel, and should therefore be evaluated from an audience perceptive than exclusively a user perspective. 
} % end of abstract
...
Important parameters for measuring good interaction techniques, with regard to communication with people, are how expressive~\cite{Brewster:2009:MIE:2227763.2227769} and natural the gestures and speech are.
...
Typical parameters that are altered in these scenarios include moving the camera, cropping the data set or applying different types of filters to the data.
...
\begin{figure*}
    \centering
    \subfigure[For varying audience sizes and display sizes the figure shows which interaction technique category is applicable. The ordering does not imply any judgement on preference of one method over another. Voice control does not depend on the either sizes.]{
...
Voice control is could be used in most variations of display and audience sizes, however, we reckon that this interaction should be used when no other technique is suitable, as it is less flexible in general. 
...
Our classifications are introduced with the purpose of not only showing status quo, but also for supporting the selection of interaction techniques for future presentation scenarios, with the use of estimated parameters such as audience size and display.
...
Such as evaluation is extensive to perform and we believe that in order to get accurate conclusions in an evaluation, only a few interaction techniques per scenario should be compared.
