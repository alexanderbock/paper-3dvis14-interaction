...
Our intention is to shift the focus from a sole analysis of the naturalness and the ease-of-use of a interaction technique, to also incorporate its {\it expressivity}, i.e., how expressive and understandable the interaction technique is when witnessed by the audience.
...
The main interaction technique used by the presenter is this setup is talking to an operator which, in turn, directly controls the visualization.
...
Instead, indirect techniques must be used, like laser pointers, rendered cursors on the screen, gesture recognition or verbal communication.
...
Furthermore, the presenter might feel less in control of the data, even neglecting the accuracy of the tracking, as he is not directly touching the display surface.
...
With this distinction in mind, most of the audience in the touchless setup ofin scenario~\ref{sec:exhibition} will see the gestures performed by the presenter as unregistered gestures, while only a few might look upon it as registered gestures.
In a presentation sense, we argue that registered gestures is preferable to unregistered gestures.
...
One issue with this system is the impossibility of mapping physical gestures to match the visualizations due to the large scale of the room and the wide differences in seating position.
It is not possible to place the presenter in such a way that his gestures would seem integrated into the visualization for all viewers simultaneously.

However, seen from a presentation perspective, the touchless gestures in this setup are more expressive then what direct touch gestures on a handheld device or a touch table would be.
Thus, the touchless setup could be considered as a better presentation tool then utilizing a touch device for remote communication, as done by Coffey~\textit{et al.}~\cite{Coffey:2012:ISW:2360744.2360843}.
