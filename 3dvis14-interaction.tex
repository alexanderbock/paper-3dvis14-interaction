...
This type of interaction is present in scenario 2 and 3. That type of interaction setups are quite similar, however both the audience size and the display size is increased in scenario 3 when compared to scenario 2. 
...
As seen in figure \ref{img:exhibition_kinect}, which is a picture of scenario 2, a hand cursor is rendered which simulates the projection of the presenters hand onto the surface. Such visual cues not only help the presenters, but also allow a wider understanding for the audience to comprehend what operation the user is currently expressing, as they can correlate position of the the display to see where the user hand is located in relation to the display. Furthermore, the icons in the bottom are not only introduced to avoid a increase in the quantity and complexity of gestures for different operations, but also to make the operations performed more in contact with the visualization. To clarify, when the user is moving is hand towards on icon, audience perceive better that something is about to happen, then if the user would perform the same operation through a gesture.
