...
%% Abstract section.
\abstract{ 
In this paper we share our experiences with different types of interaction, such as 2D direct touch and 3D tracking, in various scenarios and environments which involve not only a user, but an audience, with a novice level of expertise in the area of interaction within scientific visualization. We want with this paper to shift the focus from natural and easy-to-use interaction for the user, to how expressive and understandable the interaction technique is for the audience. While some natural exploratory interaction methods fit well for the user, they may be less beneficial when presenting the data to the audience. However, an appropriate natural interaction for the user can in many scenarios be an appropriate interaction for the audience, but our observations show that it usually highly dependent on the setting where the interaction and viewing takes place. With this in mind, we perform a comparison and relevance chart based on how beneficial and comprehensive the interaction is for the audience. The techniques we will cover are part of different scenarios where we have presented scientific data interactively for various audience size or various displays. 
} % end of abstract
