...
To further show when a certain interaction technique is preferable from an expressiveness to audience point of view we introduce a classification of the techniques covered in our scenarios. The chart in figure \ref{classifiy_diagram} show the relevance of the techniques in relation to audience and display size. As can be seen, we have chosen to split up the touchless interaction in our scenarios into \emph{registered} and \emph{unregistered} gestures, as previously detailed in section \ref{subsec:touchless}.

The conclusion which can be drawn from our mapping is that touch is suitable for interaction for small to medium display and audience sizes. 
Furthermore, \emph{registered} gestures, where the user is basically almost in contact with the display surface....\todo{Why registered gestures good when small audience, large display?}, while \emph{unregistered} are suitable when the display area and audience size is large, as not all people from the audience would be able too see the user movements close to the display area at once, and there is just to large area for touch interaction.
